...
void getLinkDensitiesFast(int *ma, int *mb, int *ea, int *eb, unsigned int *numedg, int *clusnums, int *numcl, double *pdens, double *heights, double *pdmax, int *csize, bool *removetrivial, bool *bipartite, int *bip, bool *verbose)

	{

	Rprintf("Using local C file \n");

	unsigned int i, j, k, ne, one = 0, csum = clusnums[0];
	int p = 0, nn, count = 0, rm = 0, n1, n2, which1, which2, nn0, nn1;
	int itemA = 0, itemB = 0, nodeClustIdx =0, currClustIdx = 0;
	float prog;
	...
	for(i = 0; i < *numedg-1; i++){
		...
		itemB = mergeB.at(i);
		if(itemA < 0 && itemB < 0){
			clusters[i].push_back(-1*itemA);
			clusters[i].push_back(-1*itemB);
			one = 0;
		}else if(itemA > 0 && itemB < 0){
			clusters[i] = clusters[itemA-1];
			clusters[i].push_back(-1*itemB);
			cdel.push_back(itemA-1); // Index of merged cluster so it can be deleted when possible.
			drop.push_back(itemA-1);
			one = 1;
		}else if(itemA < 0 && itemB > 0){
			clusters[i] = clusters[itemB-1];
			clusters[i].push_back(-1*itemA);
			cdel.push_back(itemB-1);
			drop.push_back(itemB-1);
			one = 1;
		}else{
			clusters[i].insert(clusters[i].end(), clusters[itemA-1].begin(), clusters[itemA-1].end());
			clusters[i].insert(clusters[i].end(), clusters[itemB-1].begin(), clusters[itemB-1].end());
			cdel.push_back(itemA-1);
			cdel.push_back(itemB-1);
			drop.push_back(itemA - 1);
			drop.push_back(itemB - 1);
			one = 2;
		...
		if(i+1 == csum){ // Reached end of this height, calculate link densities for the current clusters.
			...
			for(j = 0; j < currentTemp.size(); j++){
				// Number of edges.
				currClustIdx = currentTemp.at(j); 
				ne = clusters[currClustIdx].size();
				// Number of nodes.
				for(k = 0; k < ne; k++){
					nodeClustIdx = clusters[currClustIdx].at(k)-1;
					n1 = edgeA.at(nodeClustIdx);
					n2 = edgeB.at(nodeClustIdx);
					nodes.insert(n1);
					nodes.insert(n2);
					if(*bipartite){
						which1 = bip[(n1-1)];
